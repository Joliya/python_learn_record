...
class Solution:
    def minimumLevels(self, possible: List[int]) -> int:
        ...
        if length == 0:
            return -1
        if length == 1 and possible[0] == 0:
            return -1
        left, right = 0, length - 1
        left_score, right_score = 0, 0
        while left < right:
            if left_score < right_score:
                score = 1 if possible[left] == 1 else -1
                left_score += score
                left += 1
            else:
                score = 1 if possible[right] == 1 else -1
                right_score += score
                right -= 1
        while left_score < right_score and left < length:
            score = 1 if possible[left] == 1 else -1
            left_score += score
            left += 1
            right_score -= score
        return left if left_score > right_score else -1
...
if __name__ == '__main__':
    Solution2().minimumLevels([1, 0, 1, 0])
